# bash programmable completion for bitcoinIII-cli(1)
# Copyright (c) 2012-2022 The BitcoinIII Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $bitcoinIII-cli for RPC
_bitcoinIII_rpc() {
    ...
    $bitcoinIII_cli "${rpcargs[@]}" "$@"
}

_bitcoinIII_cli() {
    local cur prev words=() cword
    local bitcoinIII_cli

    # save and use original argument to invoke bitcoinIII-cli for -help, help and RPC
    # as bitcoinIII-cli might not be in $PATH
    bitcoinIII_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($bitcoinIII_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_bitcoinIII_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _bitcoinIII_cli bitcoinIII-cli
