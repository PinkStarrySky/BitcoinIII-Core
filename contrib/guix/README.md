# Bootstrappable BitcoinIII Core Builds

This directory contains the files necessary to perform bootstrappable BitcoinIII
Core builds.
...
To build BitcoinIII Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BitcoinIII Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitcoinIII-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitcoinIII-core/guix.sigs).
