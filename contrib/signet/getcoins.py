...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitcoinIII-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitcoinIII-cli', help='bitcoinIII-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BitcoinIII address to which the faucet should send')
...
parser.add_argument('bitcoinIII_cli_args', nargs='*', help='Arguments to pass on to bitcoinIII-cli (default: -signet)')

args = parser.parse_args()

if args.bitcoinIII_cli_args == []:
    args.bitcoinIII_cli_args = ['-signet']


def bitcoinIII_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitcoinIII_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitcoinIII_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitcoinIII_cli(['getnewaddress', 'faucet', 'bech32'])
