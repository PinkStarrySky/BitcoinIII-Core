...
Below are some notes on how to build BitcoinIII Core for Windows.

The options known to work for building BitcoinIII Core on Windows are:
...
Note that for WSL the BitcoinIII Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoinIII, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitcoinIII`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/bitcoinIII
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/bitcoinIII --strip
```
