# CJDNS support in BitcoinIII Core

It is possible to run BitcoinIII Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitcoinIII Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitcoinIII network and individual nodes.
...
before it will work with your BitcoinIII Core node. This is described in steps
...
## Run BitcoinIII Core with CJDNS

Once you are connected to the CJDNS network, the following BitcoinIII Core
...
When enabled, this option tells BitcoinIII Core that it is running in an
...
IPv6 local network. This helps BitcoinIII Core perform better address management:
...
CJDNS support was added to BitcoinIII Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitcoinIII-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bitcoinIII/bitcoinIII/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BitcoinIII Core

There are several ways to see your CJDNS address in BitcoinIII Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitcoinIII-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitcoinIII-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bitcoinIII-cli help getnodeaddresses` for details.
