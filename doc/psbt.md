# PSBT Howto for BitcoinIII Core

Since BitcoinIII Core 0.17, an RPC interface exists for Partially Signed BitcoinIII
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoinIII/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitcoinIII transactions that are not fully signed
...
#### Multisig with multiple BitcoinIII Core instances
...
BitcoinIII Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoinIII-cli` is used.
...
Later, when *V* BC3 has been received on *Amulti*, and Bob and Carol want to
