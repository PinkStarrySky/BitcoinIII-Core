...
BitcoinIII Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/bitcoinIII-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `BITCOINIII_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from BitcoinIII Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since BitcoinIII Core version 0.18.0.
...
- Testing or debugging of implementations of the BitcoinIII P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of BitcoinIII Core. BitcoinIII Core logs debug messages
...
- OpenSSL is no longer used by BitcoinIII Core.  (#17265)

- BIP70 support has been fully removed from BitcoinIII Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitcoinIII/bitcoinIII/issues/16599),
  [PR #16702](https://github.com/bitcoinIII/bitcoinIII/pull/16702), and the
  `bitcoinIIId help` for more information.  This option is experimental and
...
- All custom settings configured when BitcoinIII Core starts are now
...
- The "Start BitcoinIII Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple BitcoinIII Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed BitcoinIII Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoinIII-core/HWI) for
  signing.  Future versions of BitcoinIII Core should support a GUI option
...
- BitcoinIII Core's automatically-generated source code documentation is
...
- The `bitcoinIII-cli` utility used with the `-getinfo` parameter now
...
- The `bitcoinIII-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `bitcoinIIIstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitcoinIII-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_bitcoinIII` binary (hebasto)
...
- #18702 Fix ASLR for bitcoinIII-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitcoinIII schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitcoinIII.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitcoinIII` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitcoinIII-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitcoinIII-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about BitcoinIII Core files (hebasto)
...
- #17119 Fix broken bitcoinIII-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitcoinIII-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to bitcoinIII-wallet (hel-o)
...
- #18341 Replace remaining literal BC3 with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bitcoinIIId startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitcoinIII-cli -getinfo (malevolent)
