...
static void SetupBitcoinIIITxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitcoinIII-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitcoinIII-tx tool is used for creating and modifying bitcoinIII transactions.\n\n"
                "bitcoinIII-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitcoinIII transaction, or with \"-create [commands]\" to create a hex-encoded bitcoinIII transaction.\n"
                "\n"
                "Usage: bitcoinIII-tx [options] <hex-tx> [commands]\n"
                "or:    bitcoinIII-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinIII transaction
