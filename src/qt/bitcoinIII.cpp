...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BitcoinIIIUnit>("BitcoinIIIUnit");
#else
    qRegisterMetaType<BitcoinIIIUnit>("BitcoinIIIUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoinIII_de.qm (shortcut "de" needs to be defined in bitcoinIII.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoinIII_de_DE.qm (shortcut "de_DE" needs to be defined in bitcoinIII.qrc)
...
static const char* qt_argv = "bitcoinIII-qt";

BitcoinIIIApplication::BitcoinIIIApplication()
...
void BitcoinIIIApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitcoinIIIApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitcoinIIIGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitcoinIIIApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitcoinIIIApplication::createOptionsModel(bool resetSettings)
{
...
void BitcoinIIIApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitcoinIIIGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitcoinIIIGUI::quitRequested, this, &BitcoinIIIApplication::requestShutdown);
...
void BitcoinIIIApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitcoinIIIApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitcoinIIIApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitcoinIIIApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitcoinIIIApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitcoinIIIApplication::handleRunawayException);
    connect(this, &BitcoinIIIApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitcoinIIIApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitcoinIIIApplication::parameterSetup()
{
...
void BitcoinIIIApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitcoinIIIApplication::requestInitialize()
{
...
void BitcoinIIIApplication::requestShutdown()
{
...
void BitcoinIIIApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // bitcoinIII: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitcoinIIIGUI::handlePaymentRequest);
            connect(window, &BitcoinIIIGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitcoinIIIApplication::handleRunawayException(const QString &message)
{
...
void BitcoinIIIApplication::handleNonFatalException(const QString& message)
{
...
WId BitcoinIIIApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitcoinIIIApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitcoinIIIGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitcoinIII);
    Q_INIT_RESOURCE(bitcoinIII_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BITCOINIII_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitcoinIII-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bitcoinIII-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bitcoinIII.conf, determine network, switch to network specific
    ...
    // bitcoinIII: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
