...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_bitcoinIII", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_bitcoinIII", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_bitcoinIII", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_bitcoinIII", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_BitcoinIII Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_bitcoinIII.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("bitcoinIII_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("bitcoinIII_sock1_XXXXXX")),
    };
