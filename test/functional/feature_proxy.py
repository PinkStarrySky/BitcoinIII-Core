...
"""Test bitcoinIIId with different proxy configuration.

Test plan:
- Start bitcoinIIId's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on bitcoinIIId side:
...
class ProxyTest(BitcoinIIITestFramework):
    ...
    def node_test(self, node, *, proxies, auth, test_onion, test_cjdns):
        ...
        # Note: bitcoinIIId's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
