...
"""Class for bitcoinIIId node under test"""
...
BITCOINIIID_PROC_WAIT_TIMEOUT = 60
# The size of the blocks xor key
# from InitBlocksdirXorKey::xor_key.size()
NUM_XOR_BYTES = 8
# The null blocks key (all 0s)
NULL_BLK_XOR_KEY = bytes([0] * NUM_XOR_BYTES)
BITCOINIII_PID_FILENAME_DEFAULT = "bitcoinIIId.pid"
...
class TestNode():
    """A class for representing a bitcoinIIId node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, bitcoinIIId, bitcoinIII_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the bitcoinIII.conf file.
        # This means that starting a bitcoinIIId using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(bitcoinIII_cli, self.datadir_path)
    ...
    def __del__(self):
        # Ensure that we don't leave any bitcoinIIId processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then bitcoinIIId would bind P2P ports on
        ...
        # bitcoinIII.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time bitcoinIIId is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by bitcoinIIId, and
        ...
        self.running = True
        self.log.debug("bitcoinIIId started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the bitcoinIIId process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'bitcoinIIId exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; bitcoinIIId is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to bitcoinIIId after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; bitcoinIIId is still starting
    ...
    def wait_until_stopped(self, *, timeout=BITCOINIIID_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def replace_in_config(self, replacements):
        ...
        The substitutions are passed as a list of search-replace-tuples, e.g.
            [("old", "new"), ("foo", "bar"), ...]
        """
        with open(self.bitcoinIIIconf, 'r', encoding='utf8') as conf:
            conf_data = conf.read()
        for replacement in replacements:
            assert_equal(len(replacement), 2)
            old, new = replacement[0], replacement[1]
            conf_data = conf_data.replace(old, new)
        with open(self.bitcoinIIIconf, 'w', encoding='utf8') as conf:
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into bitcoinIIId")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to bitcoinIIId
        expected_msg: regex that stderr should match when bitcoinIIId fails

        Will throw if bitcoinIIId starts without an error.
        Will throw if an expected_msg is provided and it does not match bitcoinIIId's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'bitcoinIIId exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'bitcoinIIId should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to bitcoinIII-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.bitcoinIIIcli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with bitcoinIII-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run bitcoinIII-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running bitcoinIII-cli {}".format(p_args[2:]))
